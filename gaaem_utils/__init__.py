...
def make_XYZ_model(xyz):
    ...
    xyz["flightlines"]["alt"]=xyz["flightlines"].input_tx_height  
    if "inverted_tx_height" in xyz["flightlines"].columns:
        xyz["flightlines"]["invalt"]=xyz["flightlines"].inverted_tx_height
        
    else:
        xyz["flightlines"]["invalt"]=xyz["flightlines"].input_tx_height * np.nan
...
def copy_hrd_files(file_list):
    file_list.sort()
    header_filename='.'.join(file_list[0].split('.')[:-1])+'.hdr'
    for file in file_list[1:]:
        filename='.'.join(file.split('.')[:-1])+'.hdr'
        shutil.copy(header_filename, filename)
